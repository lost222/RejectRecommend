...
func (F *UserFeed) SaveRedis() (error) {
	...
	//var ctx = context.Background()
	//todo 过期时间
	_ , err = rdb.HSet(F.UserName, F.Rssurl, data).Result()
	_, err = rdb.Expire(F.UserName, FeedLastTime*time.Second).Result()
	if err != nil {
		return err
	}
...
func (F *UserFeed) GetRedis() error{
	...
	rebytes,err := rdb.HGet(F.UserName, F.Rssurl).Result()
	if err != nil {
		return err
	}
	//json反序列化
...
func (c *Cache) SaveInRedis() (bool, error){
	...
	res, err := rdb.Set(c.Rssurl, data, cachelastime * time.Second).Result()
	if err != nil {
		return false, err
	}
...
func (c *Cache) GetFromRedis() (bool, error){
	...
	rebytes,_ := rdb.Get(c.Rssurl).Result()
	//json反序列化
	object := &gofeed.Feed{}
	err := json.Unmarshal([]byte(rebytes),object)
	if err != nil {
...
func Getactiveusr() []string{
	...
	res, err := rdb.ZRangeByScore("activeusr", redis.ZRangeBy{
	...
	_, err = rdb.Do("ZREMRANGEBYSCORE","activeusr","-inf",mintime).Result()
...
func SetActUser(userName string) error {
	...
	_, err := rdb.ZAdd("activeusr", redis.Z{Score: float64(timeUnix), Member: userName}).Result()
